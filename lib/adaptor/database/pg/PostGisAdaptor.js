...
//    console.log("feature",feature);
	...
	for(i=0,len= fields.length;i<len;i+=1){
		...
		if( fields[i].type == 'Character' )
		{
			...
			values.push( parseInt(record[fields[i]["name"]]) );
		}
...
var select = exports.select = function select( layer ,bounds, callback ){
	var query = "SELECT ST_AsGeoJSON("+geometryColumnName+") AS geometry FROM "+layer.name;
		query = query + " WHERE "+geometryColumnName+" && ";
		query = query + "ST_MakeEnvelope("+ bounds.toBboxString() + ",3785)";
